...
const faqWrapLink = document.querySelector('.faq__wrap-link');
const faqNav = document.querySelector('.faq__nav');

faqNav.addEventListener('click', (event) => {
  if (event.target.classList.contains('faq__nav-link')) {
    // Удаляем класс tab--active у всех остальных элементов
    const activeLinks = document.querySelectorAll('.faq__nav-link.tab--active');
    activeLinks.forEach(link => link.classList.remove('tab--active'));

    // Добавляем класс tab--active к текущему элементу
    event.target.classList.add('tab--active');

    // Копируем содержимое
    faqWrapLink.innerHTML = event.target.innerHTML;
  }
});
